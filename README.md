...
**3.16.1 — Team workspace:** Long work gets a real on-disk room (`~/.pocket/teams/<id>/workspace`) that a team of agents share. Codex CLI (already on this host) and Grok run in that cwd. Long workflows attach and tick into NOTES.md. KEEP dies with chat; the team folder does not. `GET /v1/team/workspace`.
