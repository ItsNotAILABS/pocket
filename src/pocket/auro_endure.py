...
Pocket adapter. Native organism.endure is learning only when that module
exists *and* reports stateful evaluation. The Pocket-side fallback is
repeated inference: not learning, no reward selection, no endurance claim.
"""
...
def _fallback(goal: str, *, experiments: int, cycles: int) -> Dict[str, Any]:
    ...
    payload["summary"] = (
        f"Auro Endure adapter (not learning): {experiments} experiments + {cycles} cycles. "
        "Native stateful evaluation is not on this host."
    )
    return payload
...
def run(goal: str, *, experiments: int = 4, cycles: int = 6) -> Dict[str, Any]:
    ...
    try:
        ...
        r["native"] = True
        # Native may only claim learning when it actually ran stateful eval.
        r["learning"] = bool(r.get("learning") and r.get("stateful_eval"))
        r.setdefault("stateful_eval", False)
        if not r["learning"]:
            r.setdefault(
                "summary",
                "Auro Endure native ran. Not represented as learning until stateful evaluation exists.",
            )
        return r
